...
fn self_attention(
    ...
    for h in 0..n_groups * n_kv_h {
        for l in 0..seq_len {
            let q_index = l * dqkv * n_kv_h * n_groups + h * dqkv;
            for i in 0..total_seq_len {
                let mut sum = 0.0;
                let k_index = i * dqkv * n_kv_h + h / n_groups * dqkv;
                for j in 0..dqkv {
                    sum += q_data[q_index + j] * k_data[k_index + j];
                }
                att_data[h * seq_len * total_seq_len + l * total_seq_len + i] = sum * scale;
            }
        }
    }
    ...
    for h in 0..n_groups * n_kv_h {
        for l in 0..seq_len {
            ...
            for i in 0..dqkv {
                let mut sum = 0.0;
                let v_index = h / n_groups * dqkv + i;
                for j in 0..total_seq_len {
                    sum += att_data[a_index + j] * v_data[v_index + j * dqkv * n_kv_h];
                }
                data[l * dqkv * n_kv_h * n_groups + h * dqkv + i] = sum;
            }
        }
    }
