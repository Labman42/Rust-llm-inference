...
pub fn matmul_transb(c: &mut Tensor<f32>, beta: f32, a: &Tensor<f32>, b: &Tensor<f32>, alpha: f32) {
    ...
    assert_eq!(k_a, k_b, "a and b must have the same inner dimension");
    assert_eq!(c_shape[0], m, "a and c must have the same n_row");
    assert_eq!(c_shape[1], n, "b and c must have the same n_col");
    ...
    for i in 0..m {
        for j in 0..n {
            let mut sum = 0.0;
            for k in 0..k_a {
                sum += _a[i * k_a + k] * _b[j * k_b + k];
            }
            let idx_c = i * n + j;
            _c[idx_c] = beta * _c[idx_c] + alpha * sum;
        }
    }
